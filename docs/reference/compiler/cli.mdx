...
* `-V, --version`
  * Print versions for the compiler package and its associated content-bundle schema and DSL grammar, plus the absolute path of the Python interpreter running the compiler, and then exit. The output format is one `<key> <value>` field per line, suitable for both human reading and `grep`-based parsing.
* `-p, --print`
...
Log versions for the compiler, content-bundle schema, and DSL grammar associated with your installation, plus the absolute path of the Python interpreter running the compiler:
